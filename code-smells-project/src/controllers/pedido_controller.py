...
def listar_pedidos_usuario(usuario_id):
    return jsonify({"dados": pedido_model.get_by_user(usuario_id), "sucesso": True}), 200


def listar_todos_pedidos():
    return jsonify({"dados": pedido_model.get_all(), "sucesso": True}), 200
...
def relatorio_vendas():
    return jsonify({"dados": pedido_model.sales_report(), "sucesso": True}), 200
